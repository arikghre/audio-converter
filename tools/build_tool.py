...
Double-clic sur AgsMediaConverter.exe pour generer AGsMediaConverterSetup.exe
"""
import os
import sys
import shutil
...
# ROOT = repo root in both frozen (exe) and script modes
...
AllowNoIcons=yes
SetupIconFile={root}\\src\\logo.ico
...
def main():
    ...
    run([python, "-m", "pip", "install", "-r",
         os.path.join(ROOT, "src", "requirements.txt")])

    step("Construction de l'application (PyInstaller)...")
    run([python, "-m", "PyInstaller",
         os.path.join(ROOT, "tools", "AGsMediaConverter.spec"), "--noconfirm"])
